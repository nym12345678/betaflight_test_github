...
{
#ifdef USE_SPI
    spiPinConfigure(spiPinConfig(0));   //把宏定义SPI的一些配置给赋值给spiPinConfig(0)
#endif

    sensorsPreInit();   //先预先配置IO输出模式

#ifdef USE_SPI
    spiPreinit();

#ifdef USE_SPI_DEVICE_1
    spiInit(SPIDEV_1);  //STM32或AT32的SPI驱动初始化
#endif
...
/* temp until PGs are implemented. */
    ...
    if (!(initFlags & SPI_BUSSES_INIT_ATTEMPTED)) { //如果SPI_BUSSES_INIT_ATTEMPTED位没有初始化，则初始化configureSPIBusses();
        configureSPIBusses();
        initFlags |= SPI_BUSSES_INIT_ATTEMPTED; //初始化完成给initFlags对应该位赋值1;
    }
